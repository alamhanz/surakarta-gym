    ...
    def __eq__(self, obj):
        if obj is None:
            ...
            return (
                ...
                and self.to_x == obj.to_x
                and self.to_y == obj.to_y
            )
...
class Board(object):
    ...
    def __get_eat_pos(self, x, y, direction, chess, arc_count):
        ...
        if not success:
            ...
            if x == -1:
                return self.__get_eat_pos(
                    pos_list[y - 1][0], pos_list[y - 1][1], x_dir, chess, arc_count + 1
                )
            elif x == 6:
                return self.__get_eat_pos(
                    pos_list[y + 3][0], pos_list[y + 3][1], x_dir, chess, arc_count + 1
                )
            elif y == -1:
                return self.__get_eat_pos(
                    pos_list[x + 7][0], pos_list[x + 7][1], y_dir, chess, arc_count + 1
                )
            else:  # y == 6
                return self.__get_eat_pos(
                    pos_list[x + 11][0],
    ...
    def won(self):
        ...
        """
        return (
            self.__status == GameStatus.RedWon or self.__status == GameStatus.BlackWon
        )
    ...
    def get_can_eat(self, x, y, direction=None):
        """
        ...
        if direction is None:
            ...
            possible_eat = self.__get_eat_pos(x, y, direction, chess, 0)
            if possible_eat[0] is not None:
                return [possible_eat]
            else:
                return []
    ...
    def apply_action(self, action):
        ...
        if action.is_move:
            ...
            return self.player_eat(action.x, action.y, action.to_x, action.to_y)
